...
export const useScorecardLogic = (stats, setStats) => {
        ...
        if (predictionForThisRow && lastWinRow === rowIdx - 1) {
            setStats(currentStats => {
                ...
                if (oldActualWinType && oldActualWinType !== newActualWinType) {
                    ...
                    if (wasOldResultCorrect) {
                        newStats.predictions.correct = Math.max(0, newStats.predictions.correct - 1);
                    } else {
                        newStats.predictions.wrong = Math.max(0, newStats.predictions.wrong - 1);
                    }
                    if (config.capturePatternStats) {
                        const activePatterns = new Set();
                        if (lastWinRow > 0 && scorecard[lastWinRow]) {
                            for (let col = 3; col < scorecard[lastWinRow].length; col++) {
                                const cellKey = `${lastWinRow}-${col}`;
                                if (highlightedCells.has(cellKey)) { activePatterns.add(highlightedCells.get(cellKey)); }
                            }
                        }
                        activePatterns.forEach(pName => {
                            const patternData = newStats.patternStats.get(pName);
                            if (patternData) { if (wasOldResultCorrect) { patternData.wins = Math.max(0, patternData.wins - 1); } else { patternData.losses = Math.max(0, patternData.losses - 1); } }
                        });
                    }
                }
                ...
                if (config.capturePatternStats) {
                    ...
                    activePatterns.forEach(pName => {
                        const patternData = newStats.patternStats.get(pName);
                        if (patternData) { if (isNewResultCorrect) { patternData.wins++; } else { patternData.losses++; } }
                    });
                }
                return newStats;
            });
    ...
    }, [scorecard, lastWinRow, lastWinType, predictedWinType, highlightedCells, setStats, calculateSingleRow, stats]);
